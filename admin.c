...
void book_test(book_list *book_data){
    ...
    head->price=9.99;
    head->field=science;
    head->press="People's press";
    head->author_name="WW";
    head->next=next=(book *)malloc(sizeof(book));
    next->code=2;
    next->id_number=100;
    next->book_name="book2";
    next->price=19.99;
    next->field=art;
    next->press="First press";
    next->author_name="XX";
    next->next=(book *)malloc(sizeof(book));
    next=next->next;
    next->code=3;
    next->id_number=-1;
    next->book_name="book3";
    next->price=19.99;
    next->field=art;
    next->press="People's press";
    next->author_name="YY";
    next->next=(book *)malloc(sizeof(book));
    next=next->next;
    next->code=4;
    next->id_number=-1;
    next->book_name="book4";
    next->price=19.99;
    next->field=science;
    next->press="Second press";
    next->author_name="ZZ";
    next->next=NULL;
}
